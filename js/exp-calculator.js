...
(() => {
  ...
  function makeKillLevelHistoryEntry(event) {
    const isKill = event.category === "kill";
    const resultText = event.resultText || event.detail || "";
    const valueText =
      event.value === null || event.value === undefined
        ? "—"
        : `${event.value > 0 ? "+" : ""}${numberFormatter.format(event.value)} EXP`;
    ...
    return `
      <div class="exp-history-row exp-kill-row">
        ...
        <div class="exp-history-main">
          <strong>${escapeActivityHtml(event.title)}</strong>
          ${isKill && event.doubleXpUsed ? '<small class="exp-history-badge">2× EXP</small>' : ""}
        </div>
        <span class="${event.value > 0 ? "positive" : ""}">${escapeActivityHtml(valueText)}</span>
        <span class="exp-history-result">${escapeActivityHtml(resultText)}</span>
      </div>
    `;
  }
  ...
  function renderCustomActivityList(listElement, emptyElement, headerElement, events, maker) {
    ...
    if (headerElement) {
      headerElement.classList.toggle("hidden", !hasEvents);
    }
    ...
    listElement.innerHTML = events.map(maker).join("");
  }
  ...
  function renderSessionTracking() {
    ...
    if (elements.sessionProfitSummary) {
      const profit = sessionState.expGained - sessionState.expLost;
      elements.sessionProfitSummary.textContent =
        `Profit ${profit >= 0 ? "+" : "-"}${numberFormatter.format(Math.abs(profit))} EXP`;
      elements.sessionProfitSummary.classList.toggle("negative", profit < 0);
      elements.sessionProfitSummary.classList.toggle("positive", profit >= 0);
    }
  }
 ...
 function recordSessionEvent(event) {
  ...
  function recordKill(boss, gainedExp, levelBefore, levelAfter, doubleXpUsed) {
    ...
    recordSessionEvent({
      ...
      value: gainedExp,
      doubleXpUsed,
      resultText: `Level ${numberFormatter.format(levelAfter)}`
    });
