...
const STORAGE_KEYS = {
  ...
  function getHighestNormalFloors() {
    const floors =
      Object
        .values(
          window.SBO_BOSSES || {}
  ...
  function isBossExpReduced(
    ...
    // Exactly 20 levels above still gives full EXP.
    // Reduction begins at 21+ levels above.
    if (
      levelDifference <= 20
    ) {
      return false;
    ...
    // Event bosses always use the reduction rule.
    if (
      isEventBoss(
        boss
      )
    ...
    // Normal bosses on the highest 2 floors are unrestricted.
    const highestFloors =
      getHighestNormalFloors();

    if (
      highestFloors.includes(
        Number(boss.floor)
      )
    ) {
      return false;
...
function getSessionFilename(extension) {
  ...
  return `SBO-Farming-Session-${date}-${time}.${extension}`;
}


function getExportDateTime(timestamp) {
  if (!timestamp) {
    return "";
  }
...
function getExportTime(timestamp) {
  if (!timestamp) {
    return "";
  }
...
function makeOverviewSheetData() {
  const profit =
    sessionState.expGained -
    sessionState.expLost;
  ...
  return [
    ["SBO: Rebirth Farming Session"],
    [],
    ["Session Overview"],
    ["Boss Kills", sessionState.bossKills],
    ["EXP Gained", sessionState.expGained],
    ["Levels Gained", sessionState.levelsGained],
    ["Deaths", sessionState.deaths],
    ["EXP Lost", sessionState.expLost],
    ["Profit", profit],
    ["Active Time", formatActiveDuration(sessionState.activeTimeMs)],
    ["EXP / Hour", expPerHour],
    ["Kills / Hour", killsPerHour],
    [],
    ["Session Details"],
    ["Session Started", getExportDateTime(sessionState.startedAt)],
    ["Last Activity", getExportDateTime(sessionState.lastActivityAt)]
  ];
}


function makeSessionLogSheetData() {
  const events =
    [...getVisibleEvents()]
      .sort(
        (a, b) =>
...
function makeDeathHistorySheetData() {
  const deathEvents =
    getVisibleEvents()
      .filter(
        (event) =>
  ...
  deathEvents.forEach((event) => {
    ...
    rows.push([
      getExportTime(event.timestamp),
      Number(death.level) || 0,
      Number(death.expBefore) || 0,
      `${Number(death.preservationPercent) || 0}%`,
      Number(death.expLost) ||
        Math.abs(Number(event.value) || 0),
      Number(death.expAfter) || 0
    ]);
  });
...
function makeKillLevelHistorySheetData() {
  const killLevelEvents =
    getVisibleEvents()
      .filter(
        (event) =>
          event.category === "kill" ||
...
function setExportColumnWidths(
  worksheet,
  widths
) {
  worksheet["!cols"] =
...
function saveCurrentSession() {
  ...
  if (
    ...
    showError(
      "Excel export could not load. Refresh the page and try Save again."
    );
  ...
  try {
    ...
    const overviewSheet =
      window.XLSX.utils.aoa_to_sheet(
        makeOverviewSheetData()
      );

    const sessionSheet =
      window.XLSX.utils.aoa_to_sheet(
        makeSessionLogSheetData()
      );

    const deathSheet =
      window.XLSX.utils.aoa_to_sheet(
        makeDeathHistorySheetData()
      );

    const killLevelSheet =
      window.XLSX.utils.aoa_to_sheet(
        makeKillLevelHistorySheetData()
      );

    setExportColumnWidths(
      overviewSheet,
      [24, 24]
    );
    ...
    console.error(
      "Could not export Excel session:",
      error
    );
